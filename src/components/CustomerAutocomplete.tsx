...
export default function CustomerAutocomplete({
  ...
  const suggestions = useMemo(() => {
    ...
    return allClients.filter(c => 
      ...
      (c.cpfCnpj && c.cpfCnpj.toLowerCase().includes(typed))
      ...
      {isOpen && value.trim().length > 0 && (
        <div className="absolute left-0 right-0 mt-1 bg-white border border-zinc-200 rounded-xl shadow-lg z-50 max-h-60 overflow-y-auto divide-y divide-zinc-100">
          {suggestions.length > 0 ? (
                ...
                <div className="flex flex-wrap items-center gap-x-3 gap-y-0.5 text-[10px] text-zinc-400 mt-0.5">
                  ...
                  {client.cpfCnpj && (
                    <span className="flex items-center gap-1 font-mono">
                      <CreditCard size={9} /> {client.cpfCnpj}
                    </span>
                  )}
                  {client.address && (
            ...
            <div className="px-4 py-3 text-xs text-zinc-500 font-medium">
              Cliente não encontrado. Continue digitando para cadastrá-mo como novo.
            </div>
          )}
        </div>
      )}
